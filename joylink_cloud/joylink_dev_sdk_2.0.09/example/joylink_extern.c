...
joylink_key_task(void)
{
        ...
        while(1)
        {
                ...
                if(key[0] == 'c' || key[0] == 'C'){
                        ...
                        printf("> set sdk unactived mode!\n");
                }
                else if(key[0] == 's' || key[0] == 'S'){
                        getin_config_flag = 1;
                        _g_pdev->jlp.is_actived = 0;
                        printf("> set sdk config mode!\n");
                }
                else if(key[0] == 'e' || key[0] == 'E'){
                        getin_config_flag = 2;
                        _g_pdev->jlp.is_actived = 1;
                        printf("> exit current mode!\n");
                }
                else{
                        printf("> \"c\"or\"C\": set unactived mode!\n");
                        printf("> \"s\"or\"S\": set config mode!\n");
                        printf("> \"e\"or\"E\": exit current mode!\n");
                }
                memset(key, 0, 32);
        }
}
...
joylink_dev_get_jlp_info(JLPInfo_t *jlp)
{
...
#endif
	pthread_t tidp;
	int ret_fd = pthread_create(&tidp, NULL, (void *)joylink_key_task, NULL);
	if(ret_fd < 0){
		log_info("read key pthread create failed!");
	}
	...
	*/
	...
	jlp->noSnapShort = user_jlp.noSnapShort;
