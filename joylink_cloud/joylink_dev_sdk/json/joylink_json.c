...
joylink_package_scan(const char *retMsg, const int retCode, DevScan_t *scan, JLDevice_t *dv)
{
    ...
    for(i = 0; i < 128; i++)
    {
	if(scan->dev_sign[i] == 'a') scan->dev_sign[i] = 'A';
	if(scan->dev_sign[i] == 'b') scan->dev_sign[i] = 'B';
	if(scan->dev_sign[i] == 'c') scan->dev_sign[i] = 'C';
	if(scan->dev_sign[i] == 'd') scan->dev_sign[i] = 'D';
	if(scan->dev_sign[i] == 'e') scan->dev_sign[i] = 'E';
	if(scan->dev_sign[i] == 'f') scan->dev_sign[i] = 'F';
    }
	...
	if(strlen(scan->dev_sign) != 0){
		cJSON_AddStringToObject(idt, "d_s", scan->dev_sign);
	}
	else{
    ...
    if(dv->jlp.devtype == E_JLDEV_TYPE_GW){
        sdev = joylink_dev_sub_devs_get(&count);
        if(NULL != sdev){
