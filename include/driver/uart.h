 ...
 *
 * @param uart_num UART_NUM_0, UART_NUM_1
...
void uart_test(void* arg)
{
    ...
    int rx_cnt = 0;
    int tx_cnt = 0;
    int dbg_tx_fifo_cnt = 0;
    int cnt = 0;
    while(1) {
        memset(data, 0, sizeof(data));
        int len = uart_read_bytes(uart_num, data, sizeof(data), 100/portTICK_PERIOD_MS);
        rx_cnt += len;
        tx_cnt += uart_write_bytes(uart_num, data, len);
        if (cnt++ % 100 ==0) {
            printf("cnt: rx  %d;tx %d;fifo: %d\n", rx_cnt, tx_cnt, dbg_tx_fifo_cnt);
        }
    }
}
 */
