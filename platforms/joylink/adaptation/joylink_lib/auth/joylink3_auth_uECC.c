...
#if !asm_clear
uECC_VLI_API void jl3_uECC_vli_clear(uECC_word_t *vli, wordcount_t num_words) {
    wordcount_t i;
...
uECC_VLI_API void uECC_vli_mmod(uECC_word_t *result,
    ...
    uECC_word_t carry = 0;
    jl3_uECC_vli_clear(mod_multiple, word_shift);
    if (bit_shift > 0) {
...
uECC_VLI_API void uECC_vli_modInv(uECC_word_t *result,
    ...
    if (uECC_vli_isZero(input, num_words)) {
        jl3_uECC_vli_clear(result, num_words);
        return;
    }

    uECC_vli_set(a, input, num_words);
    uECC_vli_set(b, mod, num_words);
    jl3_uECC_vli_clear(u, num_words);
    u[0] = 1;
    jl3_uECC_vli_clear(v, num_words);
...
static void XYcZ_initial_double(uECC_word_t * X1,
    ...
    if (initial_Z) {
        ...
        jl3_uECC_vli_clear(z, num_words);
        z[0] = 1;
    }
...
uECC_VLI_API void uECC_vli_bytesToNative(uECC_word_t *native,
                                         ...
                                         int num_bytes) {
    wordcount_t i;
    jl3_uECC_vli_clear(native, (num_bytes + (uECC_WORD_SIZE - 1)) / uECC_WORD_SIZE);
    for (i = 0; i < num_bytes; ++i) {
        unsigned b = num_bytes - 1 - i;
...
int jl3_uECC_valid_point(const uECC_word_t *point, uECC_Curve curve) {
    uECC_word_t tmp1[uECC_MAX_WORDS];
    uECC_word_t tmp2[uECC_MAX_WORDS];
    wordcount_t num_words = curve->num_words;
...
int jl3_uECC_valid_public_key(const uint8_t *public_key, uECC_Curve curve) {
    ...
    return jl3_uECC_valid_point(public, curve);
}
...
static void bits2int(uECC_word_t *native,
    ...
    jl3_uECC_vli_clear(native, num_n_words);
...
static int uECC_sign_with_k(const uint8_t *private_key,
    ...
    if (!g_rng_function) {
        jl3_uECC_vli_clear(tmp, num_n_words);
        tmp[0] = 1;
